...
export function Dashboard({
  ...
  return (
    <div className="ak-dashboard">
      {showRange ? (
        <div className="ak-toolbar">
          ...
          <div className="ak-range">
            {DATE_RANGE_PRESETS.filter((preset) => ["24h", "7d", "30d", "90d", "12mo"].includes(preset)).map(
              (preset) => (
                <button
                  key={preset}
                  type="button"
      ...
      <div className="ak-grid" style={{ gridTemplateColumns: `repeat(${columns}, minmax(0, 1fr))` }}>
        {widgets.map((item, index) => {
          const def = getWidget(item.widget);
          if (!def) {
            return (
              <section key={`${item.widget}-${index}`} className="ak-widget" style={{ gridColumn: `span ${item.span ?? 1}` }}>
                <p className="ak-muted">Unknown widget: {item.widget}</p>
              </section>
            );
          }
          const Component = def.component;
          return <Component key={`${item.widget}-${index}`} span={item.span} {...(item.props ?? {})} />;
        })}
      </div>
    </div>
  );
